package com.kafka.learning.Final;

/**
 * Created by hadoop on 8/30/18.
 */
...
import com.kafka.learning.Lesson9.serdes.SerdesFactory;
import com.kafka.learning.Lesson9.timeextractor.OrderTimestampExtractor;
...
public class PurchaseAnalysis {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        //props.put(StreamsConfig.ZOOKEEPER_CONNECT_CONFIG, "localhost:12181/kafka0.9");
        ...
        long windowSizeMs = TimeUnit.MINUTES.toMillis(60);
        long advanceMs =    TimeUnit.SECONDS.toMillis(5);
        Produced <String, OrderUser> pStrOrderUser = Produced.with(Serdes.String(), SerdesFactory.serdFrom(OrderUser.class));

        StreamsBuilder streamBuilder = new StreamsBuilder();
        KStream<String, Order> orderStream = streamBuilder.stream("orders");
        KTable<String, User> userTable = streamBuilder.table("users");
        KTable<String, Item> itemTable = streamBuilder.table("items");
//		itemTable.toStream().foreach((String itemName, Item item) -> System.out.printf("Item info %s-%s-%s-%s\n", item.getItemName(), item.getAddress(), item.getType(), item.getPrice()));
        KTable<String, Double> kTable = orderStream
                .leftJoin(userTable, (Order order, User user) -> OrderUser.fromOrderUser(order, user))
                ...
                .through("orderuser-repartition-by-item", pStrOrderUser)
                .leftJoin(itemTable, (OrderUser orderUser, Item item) -> OrderUserItem.fromOrderUser(orderUser, item))
                .map((String itemName, OrderUserItem orderUserItem) -> new KeyValue<String, OrderUserItem>(orderUserItem.getItemType(), orderUserItem))
                .groupByKey()
                .windowedBy(TimeWindows.of(windowSizeMs).advanceBy(advanceMs))
                .aggregate()
                //.filter((String item, OrderUserItem orderUserItem) -> orderUserItem.userAddress.equalsIgnoreCase(orderUserItem.itemAddress)
                        //StringUtils.compare(orderUserItem.userAddress, orderUserItem.itemAddress) == 0
                //)
...
//		kTable.foreach((str, dou) -> System.out.printf("%s-%s\n", str, dou));
    ...
    public static class OrderUserItem {
        ...
        public double getSalesAccounts() {
    ...
    public static class PurchaseAnalysis1 {
